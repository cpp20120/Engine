...
struct tuple_element<N, T<Args...>> {
  ...
  struct impl<index_sequence<m...>, index_sequence<n...>> {
    ...
    using type =
        typeof_t<deduct<T<std::conditional_t<n == N, Args, std::void_t<>>...>>>;
  };

  using type =
      typeof_t<impl<make_index_sequence<N>, index_sequence_for<Args...>>>;
};
...
struct tuple_offset {
  static constexpr auto value =
      tuple_offset<N - 1, T>::value + sizeof(tuple_element_t<N - 1, T>);
};
...
template <auto N, typename T>
struct element : std::conditional_t<is_variadic_type_v<T>, tuple_element<N, T>,
                                    get<N, T>> {};
...
struct intersection_impl<Xs<XArgs...>, Xs<YArgs...>> {
  ...
  struct filter_types<Head, Tail...> {
    using type =
        typename std::conditional_t<(std::is_same<Head, YArgs>::value || ...),
                                    filter_types<Tail...>,
...
template <typename Xs, typename Ys>
using intersection =
    typename intersectiondetail::intersection_impl<Xs, Ys>::template type<>;
...
template <
    template <template <typename...> typename, typename...> typename transform,
...
template <
    template <template <typename...> typename, typename, int, int> typename F>
struct metaf {
...
template <template <typename...> typename F, auto M, auto N, typename T,
          typename U = T>
...
template <template <typename, typename> typename F, auto N, typename T,
          typename U>
...
template <template <typename...> typename F, typename T, auto B = 0,
          auto E = sizeof_t_v<T>>
...
template <template <typename...> typename F, typename T,
          int B = sizeof_t_v<T> - 1, int E = -1>
inline constexpr auto find_if_backward_v =
    typev<find_if_backward_t<F, T, B, E>>;
...
template <template <typename...> typename F, typename T,
          int B = sizeof_t_v<T> - 1, int E = -1>
inline constexpr auto find_if_not_backward_v =
    typev<find_if_not_backward_t<F, T, B, E>>;

template <typename T, typename U, bool D = true, auto B = 0,
          auto E = sizeof_t_v<U>,
          template <typename, typename> typename F = less_t>
struct bound {
...
struct equal_range {
  using type =
      pair_v<lower_bound_v<T, U, B, E, F>, upper_bound_v<T, U, B, E, F>>;
};
...
template <size_t N, typename T, typename... Args>
  requires(N > 0)
decltype(auto) nth_value_v(T&& t, Args&&... args) {
  return std::forward<nth_type_t<N - 1, Args...>>(
      nth_value_v<N - 1>(std::forward<Args>(args)...));
}

template <typename... Args>
decltype(auto) first_value(Args&&... args) {
  return std::forward<first_type<Args...>>(
      nth_value_v<0>(std::forward<Args>(args)...));
}

template <typename... Args>
decltype(auto) last_value(Args&&... args) {
  return std::forward<last_type<Args...>>(
      nth_value_v<sizeof_v<Args...> - 1>(std::forward<Args>(args)...));
}
...
struct index_list {
  ...
  static constexpr decltype(auto) size() { return sizeof...(values); }
};
...
struct integral_range {
  ...
  struct impl<lower, lower, values...>
      : std::type_identity<index_list<T, values...>> {};
...
template <typename T, T begin, T end>
using range_of =
    rename_t<integral_range_t<T, begin, end>, std::integer_sequence<T>>;
...
using reverse_index_sequence =
    rename_t<integer_range<N - 1, -1>, std::index_sequence<>>;
...
struct invoker : base<typev<T>, typeof_t<T>>... {
  ...
  invoker(F&& f, Args&&... args)
      : base<typev<T>, typeof_t<T>>(std::forward<F>(f),
                                    std::forward<Args>(args))... {}
};

template <typename T, typename Indices, typename... Outer>
void apply_invoker(T&& t, Indices&& indices, Outer&&... outer) {
  []<typename F, template <typename U, auto...> typename V, size_t... N,
     typename... Args>(F&& f, const V<size_t, N...>&, Args&&... args) {
    invoker<index_type<N, nth_type_t<N, Args...>>...>(
        std::forward<F>(f), (nth_value_v<N>(std::forward<Args>(args)...))...);
  }(std::forward<T>(t), std::forward<Indices>(indices),
    std::forward<Outer>(outer)...);
}

template <typename T, typename... Outer>
void fmap(T&& t, Outer&&... outer) {
  apply_invoker(std::forward<T>(t), index_range<0, sizeof_v<Outer...>>(),
                std::forward<Outer>(outer)...);
}

template <typename T, typename... Outer>
void reverse_fmap(T&& t, Outer&&... outer) {
  apply_invoker(std::forward<T>(t),
                reverse_index_sequence<sizeof_v<Outer...>>(),
                std::forward<Outer>(outer)...);
}
...
struct concat<T<Args...>, T<args...>>
    : std::type_identity<T<Args..., args...>> {};

template <template <typename, auto...> typename T, typename U, auto... Args,
          auto... args>
struct concat<T<U, Args...>, T<U, args...>>
    : std::type_identity<T<U, Args..., args...>> {};

template <template <typename...> typename T, typename... Args, typename... args,
          typename... U>
struct concat<T<Args...>, T<args...>, U...>
    : concat<T<Args..., args...>, U...> {};

template <template <typename, auto...> typename T, typename U, auto... Args,
          auto... args, typename... V>
struct concat<T<U, Args...>, T<U, args...>, V...>
    : concat<T<U, Args..., args...>, V...> {};
...
template <template <typename...> typename T, typename... Args>
struct flat<T<Args...>> : concat<type_if<is_variadic_pack_v<Args>, flat<Args>,
                                         std::type_identity<T<Args>>>...> {};
...
struct unique<T<Args...>> {
  ...
  template <template <typename...> typename U, typename... prev, typename V,
            typename... args>
  struct impl<U<prev...>, V, args...> {
    using type =
        dedup_t<contains<V, prev...>, U<V>, impl<U<prev..., V>, args...>>;
  };
...
struct unique<T<U, values...>>
    : unpack<concat_t, typeof_t<unique<std::tuple<T<U, values>...>>>> {
  /*
  ...
  template <template <typename, auto ...> typename V, typename W, auto... prev,
  auto value, auto... args> struct impl<V<W, prev...>, value, args...>
  {
      using type = dedup_t<comprise<value, prev...>, V<W, value>, impl<V<W,
  prev..., value>, args...>>;
  };
...
struct unique_index<T<Args...>> {
  ...
  struct impl<N, U, args...> {
    using type = dedup_t<contains<U, args...>, std::index_sequence<N>,
                         impl<N + 1, args...>>;
  };
...
struct unique_index<T<U, values...>> {
  ...
  struct impl<N, value, args...> {
    using type = dedup_t<comprise<value, args...>, std::index_sequence<N>,
                         impl<N + 1, args...>>;
  };
...
template <bool B, template <typename, auto...> typename T, typename U,
          auto... Args, typename... args>
struct push<B, T<U, Args...>, args...> {
  using type = std::conditional_t<B, T<U, typev<args>..., Args...>,
                                  T<U, Args..., typev<args>...>>;
};

template <bool B, template <auto...> typename T, auto... Args, typename... args>
struct push<B, T<Args...>, args...> {
  using type = std::conditional_t<B, T<typev<args>..., Args...>,
                                  T<Args..., typev<args>...>>;
};
...
template <bool B, typename T, typename... Args>
using prepend_if =
    std::conditional_t<!B, std::type_identity<T>, prepend<T, Args...>>;
...
template <bool B, typename T, typename... Args>
using append_if =
    std::conditional_t<B, append<T, Args...>, std::type_identity<T>>;
...
consteval decltype(auto) make_sequence() {
  if constexpr (N == 0)
    ...
    return []<bool B, size_t... indices>(index_sequence<indices...>) {
      if constexpr (R)
        return prepend_if_t<
            B, index_sequence<(sizeof...(indices) + indices)..., indices...>,
            c_<sizeof...(indices) * 2>>();
      else
        return append_if_t<
            B, index_sequence<indices..., (sizeof...(indices) + indices)...>,
            c_<sizeof...(indices) * 2>>();
...
template <template <typename, typename> typename F, typename T, auto B = 0,
          auto E = sizeof_t_v<T>>
struct adjacent_difference {
  ...
  struct next {
    using curr = get<i, T>;
    using type =
        typeof_t<next<i + 1, j, curr, append_t<V, typeof_t<F<curr, U>>>>>;
  };
...
template <template <typename...> typename comparator, auto i, auto j,
...
template <typename T, typename... Args>
struct invert_type<invert_type<Args...>, T>
    : std::type_identity<invert_type<Args..., T>> {};
...
struct invert_value {
  ...
  struct impl<T, value, values...> {
    using type =
        typeof_t<impl<typeof_t<impl<invert_value<>, values...>>, value>>;
  };
  ...
  template <auto value, auto... values>
  struct impl<invert_value<values...>, value>
      : std::type_identity<invert_value<values..., value>> {};
...
template <typename F, typename T, typename U, typename... Args>
struct fold_right_<F, T, U, Args...>
    : fold_right_<F, call_t<F, U, T>, Args...> {};
...
template <template <auto, typename> typename F,
          template <typename...> typename T, typename... Args, auto... N,
          auto V>
...
struct expand<F, T<Args...>, std::index_sequence<N...>, true> {
  ...
  using type = std::integer_sequence<has_value_type_t<head>,
                                     sizeof_t_v<typeof_t<F<N, args>>>...>;
};
...
template <template <auto, typename> typename F,
          template <typename...> typename T, auto... M, typename... Args,
          auto... N>
...
struct expand<F, T<U, values...>, std::index_sequence<N...>, V> {
  ...
  using type = type_if<is_bool_type_v<head>, next<args, false>,
                       impl<args, sizeof...(values) != 1 || value>>;
};
...
struct convert
    : std::conditional_t<is_tuple_v<T> == B, expand_of<T, indices, true>,
                         std::type_identity<T>> {};
...
inline constexpr auto swap_v = i == N   ? j
                               : j == N ? i
                                        : N;
...
struct within {
  ...
  struct impl {
    using cond =
        std::conditional_t<i + 1 >= upper, std::false_type, impl<i + 1>>;
...
template <auto i, auto j, typename T, typename U, typename V,
          template <typename...> typename F = std::is_same>
struct gsub {
...
template <auto i, auto j, auto M, auto N, typename V,
          template <typename...> typename F = std::is_same>
...
template <auto i, auto j, typename T, typename U,
          ...
          template <typename...> typename G = std::is_same>
struct adjust {
  template <auto N, typename W, typename X = element<N, W>,
            typename Y = typeof_t<X>>
...
template <auto i, auto j, typename U, template <typename...> typename F,
          template <typename...> typename G>
struct adjust_if {
...
using change_if =
    std::conditional_t<!B, std::type_identity<U>, change<i, T, U>>;
...
struct exchange_with {
  ...
  struct impl<std::index_sequence<n, N...>, W>
      : impl<std::index_sequence<N...>, change_t<n, element_t<n, T>, W>> {};
...
using range =
    invoke_of<T, std::type_identity<
                     expand_of<T, index_sequence_of_c<upper - lower>, lower>>>;
...
using range_if =
    std::conditional_t<B, range<lower, upper, T>, std::type_identity<T>>;
...
struct swap_extent {
  ...
  using swap = concat_t<range_t<0, b1, X>, range_t<b2, e2, Y>,
                        range_t<e1, sizeof_t_v<X>, X>>;
...
template <template <typename, typename> typename F, typename T, typename U,
          auto B1 = 0, auto E1 = sizeof_t_v<T>, auto B2 = 0,
          auto E2 = sizeof_t_v<U>>
...
struct set_difference {
  ...
  struct impl {
    ...
    using type = typeof_t<impl<i + (cond1 || cond2), j, p + !cond1, q,
                               append_if_t<cond1, V, lhs>>>;
  };
...
using set_symmetric_difference_t =
    typeof_t<set_symmetric_difference<F, T, U, B1, E1, B2, E2>>;
...
struct mismatch {
  ...
  struct impl {
    ...
    using type = type_if<value, impl<i + 1, j, p + 1, q>,
                         std::type_identity<pair_v<i, p>>>;
  };
...
template <template <typename, typename> typename F, typename T, typename U,
          auto B1 = 0, auto E1 = sizeof_t_v<T>, auto B2 = 0,
          auto E2 = sizeof_t_v<U>>
inline constexpr auto mismatch_first =
    first_v<mismatch_t<F, T, U, B1, E1, B2, E2>>;

template <template <typename, typename> typename F, typename T, typename U,
          auto B1 = 0, auto E1 = sizeof_t_v<T>, auto B2 = 0,
          auto E2 = sizeof_t_v<U>>
inline constexpr auto mismatch_second =
    second_v<mismatch_t<F, T, U, B1, E1, B2, E2>>;
...
struct includes {
  ...
  struct impl {
    ...
    using type =
        type_if<i == j || cond1, std::false_type, impl<i + 1, j, p + cond2, q>>;
  };
...
struct search {
  ...
  struct impl {
    ...
    struct next {
      ...
      struct call {
        using type =
            type_if<!unary_v<F, p, q, T, U>, impl<i + 1>, next<p + 1, q + 1>>;
      };
...
template <template <typename, typename> typename F, int N, typename T,
          typename U, auto B = 0, auto E = sizeof_t_v<U>>
struct search_n {
  template <int i, int j>
  struct impl {
    ...
    struct next {
      ...
      struct call {
        using type =
            type_if<!lcall_v<F, l, U, T>, impl<l + 1, j>, next<l, p, q + 1>>;
      };
...
struct lca {
  ...
  struct impl {
    ...
    using type =
        type_if<std::is_same_v<lhs, rhs>, std::type_identity<lhs>, next>;
  };
...
template <template <typename...> typename F,
          template <typename, auto...> typename T, typename U, auto... Args>
...
template <template <typename...> typename F, typename T, auto B = 0,
          auto E = sizeof_t_v<T>>
struct generate : concat<range_t<0, B, T>, transform_t<F, range_t<B, E, T>>,
                         range_t<E, sizeof_t_v<T>, T>> {};
...
template <template <typename, typename> typename F, typename T, typename init,
          auto B = 0, auto E = sizeof_t_v<T>>
...
template <template <typename, typename> typename F, typename T,
          template <typename> typename U, typename init = visit_t<U, 0, T>,
          auto B = 0, auto E = sizeof_t_v<T>>
struct transform_inclusive_scan {
  using type =
      inclusive_scan_t<F, transform_t<U, range_t<B, E, T>>, init, B, E>;
};

template <template <typename, typename> typename F, typename T,
          template <typename> typename U, typename init = visit_t<U, 0, T>,
          auto B = 0, auto E = sizeof_t_v<T>>
using transform_inclusive_scan_t =
    typeof_t<transform_inclusive_scan<F, T, U, init, B, E>>;

template <template <typename, typename> typename F, typename T,
          template <typename> typename U, typename init, auto B = 0,
          auto E = sizeof_t_v<T>>
struct transform_exclusive_scan {
  using type =
      exclusive_scan_t<F, transform_t<U, range_t<B, E, T>>, init, B, E>;
};

template <template <typename, typename> typename F, typename T,
          template <typename> typename U, typename init, auto B = 0,
          auto E = sizeof_t_v<T>>
using transform_exclusive_scan_t =
    typeof_t<transform_exclusive_scan<F, T, U, init, B, E>>;
...
template <template <typename, typename> typename F1,
          template <typename, typename> typename F2, typename T, typename U,
          typename V, auto B1 = 0, auto E1 = sizeof_t_v<T>, auto B2 = 0>
  requires is_variadic_pack_v<T, U>
...
template <template <typename, typename> typename F1,
          template <typename, typename> typename F2, typename T, typename U,
          typename V, auto B1 = 0, auto E1 = sizeof_t_v<T>, auto B2 = 0>
inline constexpr auto inner_product_v =
    typev<inner_product_t<F1, F2, T, U, V, B1, E1, B2>>;

template <template <typename, typename> typename F, typename T, typename U,
          auto B = 0, auto E = sizeof_t_v<U>>
struct accumulate {
...
template <template <typename...> typename F, typename T, typename U, auto B = 0,
          auto E = sizeof_t_v<U>>
struct iota {
...
struct apply_permutation
    : expand_of<reverse_if_t<B, T>, reverse_if_t<B, indices>> {};
...
template <template <typename...> typename F, typename T, auto B, auto E,
          bool value>
struct copy {
  template <int i, int j, typename U>
  struct impl {
    ...
    using type =
        typeof_t<impl<i + 1, j, append_if_t<value == typev<F<curr>>, U, curr>>>;
  };
...
struct exclude
    : expand_of<T, set_difference_t<less_t, index_sequence_of_t<T>, indices>> {
};
...
template <auto i, template <typename...> typename T, typename... Args,
          typename... args>
struct insert<i, T<Args...>, args...> {
  using curr = T<Args...>;
  using type = concat_t<range_t<0, i, curr>, T<args...>,
                        range_t<i, sizeof_t_v<curr>, curr>>;
};

template <auto i, template <typename, auto...> typename T, typename U,
          auto... Args, typename... args>
struct insert<i, T<U, Args...>, args...> {
  using curr = T<U, Args...>;
  using type = concat_t<range_t<0, i, curr>, T<U, typev<args>...>,
                        range_t<i, sizeof_t_v<curr>, curr>>;
};
...
template <auto i, auto j, template <typename...> typename F, typename T,
          typename U>
  requires is_variadic_pack_v<T, U>
struct extract {
...
using append_range_if =
    type_if<B, append_range<T, U, i, j>, std::type_identity<T>>;
...
struct replace : concat<append_t<range_t<0, i, T>, Args...>,
                        range_t<j, sizeof_t_v<T>, T>> {};
...
template <auto i, auto j, template <typename...> typename F, typename T,
          typename U>
using reverse_replace_if_not_t =
    typeof_t<reverse_replace_if_not<i, j, F, T, U>>;
...
template <auto i, auto j, typename T, typename U, auto B = 0,
          auto E = sizeof_t_v<U>>
  requires is_variadic_pack_v<T, U>
...
template <bool B, template <typename...> typename T, typename U,
          typename... Args>
...
struct to_nest {
  ...
  template <template <typename...> typename U, typename V, typename W,
            typename... Args>
...
template <bool B, typename T>
using nest_clear_if =
    std::conditional_t<B, nest_clear<T>, std::type_identity<T>>;
...
template <typename T, typename U>
struct nest_set
    : to_nest<append_t<range_t<0, nest_depth_v<T>, to_flat_t<T>>, U>> {};
...
template <bool B, typename T, typename U>
using nest_set_if =
    std::conditional_t<B, nest_set<T, U>, std::type_identity<T>>;
...
template <template <auto, typename> typename F, auto N, typename T,
          bool B = true>
struct nest_apply {
...
template <template <typename...> typename F, typename T, typename U = T,
          typename... Args>
...
constexpr size_t typeindex() {
  return index_of_apply<
             std::is_same,
             std::conditional_t<strip, std::remove_cvref_t<Args>, Args>...>() -
         1;
}
...
constexpr size_t search_index(S&& s, T&& t, Args&&... args) {
  if constexpr (sizeof...(Args) == 0)
    ...
    return s == t
               ? 0
               : search_index(std::forward<S>(s), std::forward<Args>(args)...) +
                     1;
}
...
template <typename T, template <typename...> typename U, typename V,
          typename... Args>
struct type_index<T, U<V, Args...>>
    : index_t<typev<type_index<T, U<Args...>>> + 1> {};
...
template <typename T, template <typename...> typename U, typename... Args>
struct type_index_not<T, U<T, Args...>>
    : index_t<typev<type_index_not<T, U<Args...>>> + 1> {};
...
using type_index_if =
    std::conditional_t<!neg, type_index<T, U>, type_index_not<T, U>>;
...
template <auto value, template <typename, auto...> typename T, typename U,
          auto... Args>
struct value_index<value, T<U, Args...>>
    : type_index<c_<value, U>, std::tuple<c_<Args, U>...>> {};
...
template <auto value, template <typename, auto...> typename T, typename U,
          auto... Args>
struct value_index_not<value, T<U, Args...>>
    : type_index_not<c_<value, U>, std::tuple<c_<Args, U>...>> {};

template <auto value, typename T>
inline constexpr auto value_index_not_v = typev<value_index_not<value, T>>;

template <bool neg, auto value, typename T>
using value_index_if =
    std::conditional_t<!neg, value_index<value, T>, value_index_not<value, T>>;
...
template <typename T, typename U, auto B = 0, auto E = sizeof_t_v<U>>
inline constexpr auto find_not_backward_v =
    typev<find_not_backward<T, U, B, E>>;

template <template <typename...> typename T, typename U, auto B = 0,
          auto E = nest_depth_v<U>>
...
template <template <typename...> typename T, typename U, auto B = 0,
          auto E = nest_depth_v<U>>
struct nest_find_backward
    : find_backward<T<>, to_flat_t<nest_clear_t<U>>, B, E> {};

template <template <typename...> typename T, typename U, auto B = 0,
          auto E = nest_depth_v<U>>
inline constexpr auto nest_find_backward_v =
    typev<nest_find_backward<T, U, B, E>>;
...
template <template <typename...> typename T, typename U, auto B = 0,
          auto E = nest_depth_v<U>>
struct nest_find_not_backward
    : find_not_backward<T<>, to_flat_t<nest_clear_t<U>>, B, E> {};

template <template <typename...> typename T, typename U, auto B = 0,
          auto E = nest_depth_v<U>>
inline constexpr auto nest_find_not_backward_v =
    typev<nest_find_not_backward<T, U, B, E>>;
...
struct find_index {
  ...
  struct impl
      : impl<N + 1, tail_t<U>, append_if_t<visit_v<F, 0, U>, V, c_<N>>> {};
...
template <typename T, typename U>
struct find_all_not
    : find_index_not<bind_front<std::is_same, T>::template apply, U> {};
...
struct match {
  ...
  struct impl {
    ...
    using next = impl<N + 1, tail_if_t<value, V>, tail_t<W>,
                      append_if_t<value, X, c_<N>>>;

    using type =
        type_if<equal_v<V, W> && !std::is_same_v<V, W>, clear<X>, next>;
  };
...
struct subranges {
  ...
  template <int i, int j, int k, typename U, bool = i<n - 1> struct impl {
    static constexpr auto l = e / n + !!k;
    using type =
        typeof_t<impl<i + 1, j + l, k - !!k, append_t<U, subset_t<j, l, T>>>>;
  };
...
template <auto N, typename T>
using element_size =
    std::conditional_t<negav<is_tuple<T>>, get<N, T>, tuple_element_size<N, T>>;
...
struct binary_search {
  ...
  struct search {
    ...
    using next =
        std::conditional_t < value<key, search<half + 1, j>, search<i, half>>;
...
struct ternary_search {
  ...
  template <size_t l, size_t m, size_t r, auto lower,
            template <size_t, size_t> typename search>
  struct impl {
    ...
    using next = ternary_conditional_t < key < upper,
          key<lower, left, middle, right>;
  ...
  struct search {
    ...
    using type =
        type_if<lower == key, std::true_type, impl<l, mid, r, lower, search>>;
  };
...
struct exponential_search {
  ...
  template <int i, bool = i<E> struct search {
...
template <typename T, typename U, auto B = 0, auto E = sizeof_t_v<U>>
inline constexpr auto exponential_search_v =
    typev<exponential_search_t<T, U, B, E>>;
  ...
  struct search {
    ...
    struct impl {
      ...
      using next = std::conditional_t <
                   value<key, search<pos + 1, j>, search<i, pos - 1>>;
      using type = type_if<value == key, std::true_type, next>;
    };
...
template <typename T, typename U, auto B = 0, auto E = sizeof_t_v<U>>
inline constexpr auto interpolation_search_v =
    typev<interpolation_search_t<T, U, B, E>>;
...
struct fibonacci_search {
  ...
  struct search {
    ...
    using next =
        std::conditional_t <
        value<key, search<j - i, i, j, pos>, search<i * 2 - j, j - i, i, l>>;
    using type = type_if<value == key, std::true_type, next>;
  };
  ...
  template <int i, int j, int k, bool = k<E> struct impl : impl<j, k, j + k> {};
...
template <typename T, typename U, auto B = 0, auto E = sizeof_t_v<U>>
inline constexpr auto fibonacci_search_v =
    typev<fibonacci_search_t<T, U, B, E>>;
...
struct max_subarray {
  ...
  struct impl {
    ...
    using type = typeof_t<
        impl<B2 ? curr : i, curr, B2 ? q : k, B2 ? n : l, q, n + 1, p>>;
  };
...
bool any_visit(const overload_set<Args...>& f, A&& a) {
  return (... || any_visit<element_t<0, function_traits_t<Args>>>(
                     f, std::forward<A>(a)));
}
...
template <typename... Args>
using overload_sequence_selector_t =
    typeof_t<overload_sequence_selector<Args...>>;
...
struct overload_sequence {
  ...
  template <typename... Args>
  constexpr std::invoke_result_t<selector_t<Args...>, Args...> operator()(
      Args&&... args) {
    return std::invoke(std::get<selector_v<Args...>>(sequence),
                       std::forward<Args>(args)...);
  }
...
constexpr void for_pack(F&& f) {
  [&]<auto... N>(const std::index_sequence<N...>&) {
    if constexpr ((is_variadic_type_v<Args> && ...))
      (call_operator<N>(
           [&]<auto n, template <typename...> typename T, typename... args>(
               const T<args...>&) { for_type<args...>(std::forward<F>(f)); },
           Args()),
       ...);
    else
      (call_operator<N>(
           [&]<auto n, template <typename, auto...> typename T, typename U,
               auto... args>(const T<U, args...>&) {
...
constexpr decltype(auto) tuple_get(size_t i, std::tuple<Args...>& t) {
  return [&]<typename F, size_t... N>(
             F&& f, std::index_sequence<N...>) -> decltype(auto) {
...
class tuple_iterator {
 public:
  ...
  bool operator!=(const tuple_iterator& rhs) { return !(*this == rhs); }

  decltype(auto) operator*() { return tuple_get(i, t); }
...
class to_range {
 public:
  ...
  decltype(auto) begin() { return tuple_iterator(0, t); }

  decltype(auto) end() { return tuple_iterator(std::tuple_size_v<T>, t); }

  decltype(auto) operator[](size_t i) { return tuple_get(i, t); }
...
template <template <typename, auto...> typename T, typename U, typename V,
          auto... X, auto... Y>
...
struct compose {
  compose(Invokable&&... invoker)
      : invoker(std::forward<Invokable>(invoker)...) {}
  ...
  constexpr decltype(auto) apply(Args&&... args) const {
    if constexpr (N == 0)
      ...
      return apply<N - 1>(
          std::invoke(std::get<N>(invoker), std::forward<Args>(args)...));
  }
...
struct tokenize {
  template <int i, typename T,
            bool =
                i<base> struct impl : impl<i / base, prepend_c<T, i % base>> {};
...
template <int N>
struct digit {
  template <int i, bool = i<10> struct impl {
...
struct hb {
  ...
  static constexpr int value =
      (m ? n / 2 : 0) + typev<hb<(m ? m : N), (m ? n - n / 2 : n / 2)>>;
};
...
template <template <bool...> typename T, bool... B,
          template <typename...> typename U, typename... Args>
  requires(sizeof...(B) == sizeof_v<Args...>)
struct choose<T<B...>, U<Args...>>
    : concat<std::conditional_t<B, std::tuple<Args>, std::tuple<>>...> {};

template <template <bool...> typename T, bool... B,
          template <typename, auto...> typename U, typename V, auto... Args>
  requires(sizeof...(B) == sizeof...(Args))
struct choose<T<B...>, U<V, Args...>>
    : concat<std::conditional_t<B, U<V, Args>, U<V>>...> {};
...
void iterate(F&& f,
             std::integer_sequence<std::common_type_t<decltype(N)...>, N...>) {
...
struct to_int<T, value, bool_sequence<b, bs...>>
    : c_<typev<
             to_int<T, static_cast<T>((value << 1) | b), bool_sequence<bs...>>>,
         T> {};
...
struct clamp<N, std::integer_sequence<T, values...>> {
  ...
  using type =
      prepend_t<typeof_t<clamp<N - 1, tail_t<indices>>>, get<0, indices>>;
};
...
struct advance<state, std::index_sequence<tap, taps...>>
    : bool_<get_v<tap, state> ^
            typev<advance<state, std::index_sequence<taps...>>>> {};
...
struct lfsr {
  ...
  using next =
      lfsr<clamp_t<state::size(), prepend_t<state, advance<state, taps>>>,
           taps>;
};
...
struct nth_lfsr<N, lfsr<state, taps>> {
  ...
  using type = prepend_t<typeof_t<nth_lfsr<N - 1, typename call::next>>,
                         c_<invoke_v<call, unsigned>>>;
};
...
template <int p, int r, typename T,
          typename U = lfsr<bool_sequence<true, false, true, true, false>,
...
struct nest_assign {
  ...
  using type =
      nest_set_if_t<value, nest_fill_t<depth, U>, nest_last_if_t<value, T>>;
};
...
struct repeat_range {
  ...
  struct impl
      : impl<i + 1, j, concat_t<T, rename_t<fill_c<i + 1, i + 1>, base>>> {};
...
struct rotate {
  ...
  using type = concat_t<range_t<0, i, T>, extend<j, k>, extend<i, j>,
                        range_t<k, sizeof_t_v<T>, T>>;
};
...
struct parity
    : expand_of<T, integer_sequence_t<size_t, (sizeof_t_v<T> + N) / 2, B, 2>> {
};
...
struct divvy {
  ...
  template <
      ...
      bool =
          i *
          N<size> struct impl : impl<
              i + 1,
              append_t<U, type_if<B, element<i * N, T>,
                                  subset<i * N, min_v<N, size - i * N>, T>>>> {
  };
  ...
  using type =
      typeof_t<impl<0, std::conditional_t<B, clear_t<T>, std::tuple<>>>>;
};
...
struct axial_symmetry {
  ...
  template <auto N>
  struct impl<N, true>
      : concat<append_t<range_t<N / 2 + 1, N, T>, midpoint_t<T>>,
               range_t<0, N / 2, T>> {};
...
constexpr decltype(auto) transform_as_tuple(F&& f,
  ...
  return [&]<size_t... N>(const std::index_sequence<N...>&) {
    return std::tuple<std::result_of_t<F(Args)>...>(
...
struct nary {
  ...
  struct impl<N, U<Args...>> {
    using type =
        type_if<std::is_invocable_v<F, Args...>, c_<N>, nary<F, head_t<T>>>;
  };
...
template <template <typename, auto...> typename T, typename V, auto... v,
          typename U>
...
template <template <auto, typename> typename F, typename T,
          typename indices = index_sequence_of_t<T>>
...
class luple {
 public:
  constexpr luple() {
    lambda([](storage<Args>&... s) { (..., s.init()); });
  }

  template <typename... Brgs,
            decltype((..., new(nullptr) Args(std::declval<Brgs>()))) = nullptr>
  constexpr luple(Brgs&&... brgs) {
    lambda(
        [&](storage<Args>&... s) { (..., s.init(std::forward<Brgs>(brgs))); });
  }
  ...
  constexpr decltype(auto) get() {
    return [&]<auto... n>(std::index_sequence<n...>) {
      return lambda([](nil<n>..., auto& e, auto...) -> auto& { return *e; });
    }(std::make_index_sequence<N>());
  }

  ~luple() {
    lambda([](storage<Args>&... s) { (..., s->~Args()); });
  }
 ...
 private:
  ...
  class alignas(T) storage {
   public:
    ...
    constexpr void init(Brgs&&... args) {
      new (reinterpret_cast<T*>(std::addressof(data)))
          T(std::forward<Brgs>(args)...);
    }

    T* operator->() { return reinterpret_cast<T*>(std::addressof(data)); }
  ...
  static constexpr decltype(auto) to_lambda(Brgs... brgs) {
    return [=](auto f) mutable -> decltype(auto) { return f(brgs...); };
  }

  mutable decltype(to_lambda(storage<Args>()...)) lambda =
      to_lambda(storage<Args>()...);
};
...
struct luple_element<N, luple<Args...>> {
  using type = std::remove_cvref_t<
      decltype(std::declval<luple<Args...>>().template get<N>())>;
};
...
template <typename T, template <typename, bool> typename comp, bool b,
          typename... Args>
using extreme = std::conditional_t<!sizeof_v<Args...>, std::type_identity<T>,
                                   comp<tuple_t<T, Args...>, b>>;

template <typename T, typename U, bool B>
using evaluate =
    std::conditional_t<B && less_v<T, U> || !B && less_v<U, T>, T, U>;
...
template <template <typename...> typename T, bool B, typename U, typename V,
          typename... Args>
struct minmax<T<U, V, Args...>, B>
    : extreme<evaluate<U, V, B>, minmax, B, Args...> {};
...
struct minmax<T<U, values...>, B> {
  ...
  using type = transform_apply_t<folded_t, comp, to_tuple_t<T<U, values...>>,
                                 get<0, T<U, values...>>>;
};
...
constexpr decltype(auto) tuple_zip(Args&&... args) {
  auto row = [&]<size_t n>() {
    return std::make_tuple(std::get<n>(std::forward<Args>(args))...);
  };
  using indices =
      std::index_sequence<std::tuple_size_v<std::remove_cvref_t<Args>>...>;
...
constexpr decltype(auto) tuple_transpose(const std::tuple<Args...>& t) {
  return tuple_apply(t, []<typename... U>(U&&... u) {
    return tuple_zip(std::forward<U>(u)...);
  });
}
  ...
  return [&]<typename T>(T&& t) {
    ...
    if constexpr (size == 0)
      ...
      return [&]<auto... N>(const std::index_sequence<N...>&) {
        ...
        if constexpr (std::is_same_v<decltype(call_operator<0>(g)),
                                     std::void_t<>>)
          (..., call_operator<N>(g));
        else
...
struct find_max_crossing_subarray {
  ...
  struct impl {
    ...
    using type =
        typeof_t<impl<value, B ? value : max, B ? i : index, i + k, j, k>>;
  };
  ...
  using type = std::integer_sequence<int, first_v<left>, first_v<right>,
                                     second_v<left> + second_v<right>>;
};

template <int low, int mid, int high, typename T>
using find_max_crossing_subarray_t =
    typeof_t<find_max_crossing_subarray<low, mid, high, T>>;
...
struct find_maximum_subarray {
  ...
  struct search {
    ...
    using right_t =
        std::integer_sequence<int, right_low, right_high, right_sum>;
    using cross_t =
        std::integer_sequence<int, cross_low, cross_high, cross_sum>;
...
struct count {
  ...
  using type = transform_apply_t<folded_t, comp, to_tuple_t<range_t<B, E, U>>,
                                 index_type<0, T>>;
};
...
struct count_if {
  ...
  using type =
      transform_apply_t<folded_t, comp, to_tuple_t<range_t<B, E, T>>, c_0>;
};
...
struct mode {
  ...
  template <typename U>
  using unique = transform_apply_t<currying_t, impl<U>::template apply,
                                   to_tuple_t<unique_t<U>>>;
...
template <template <typename...> typename F, template <typename> typename P,
          typename T, typename U, auto B, auto E, bool W>
struct transform_of {
  template <int i, int j, typename V>
  struct impl {
...
template <template <typename...> typename F, template <typename> typename P,
          typename T, typename U, auto B = 0, auto E = sizeof_t_v<U>>
...
template <template <typename...> typename F, template <typename> typename P,
          typename T, typename U, auto B = 0, auto E = sizeof_t_v<U>>
struct reverse_transform_if
    : reverse<transform_if_t<F, P, T, reverse_t<U>, B, E>> {};
...
struct transform_if_not
    : transform_if<F, negaf<P>::template apply, T, U, B, E> {};
...
template <template <typename...> typename F, template <typename> typename P,
          typename T, typename U, auto B = 0, auto E = sizeof_t_v<U>>
struct reverse_transform_if_not
    : reverse<transform_if_not_t<F, P, T, reverse_t<U>, B, E>> {};

template <template <typename...> typename F, template <typename> typename P,
          typename T, typename U, auto B = 0, auto E = sizeof_t_v<U>>
using reverse_transform_if_not_t =
    typeof_t<reverse_transform_if_not<F, P, T, U, B, E>>;
...
template <template <typename...> typename F, template <typename> typename P,
          typename T, typename U, auto B = 0, auto E = sizeof_t_v<U>>
struct reverse_transform_while
    : reverse<transform_while_t<F, P, T, reverse_t<U>, B, E>> {};

template <template <typename...> typename F, template <typename> typename P,
          typename T, typename U, auto B = 0, auto E = sizeof_t_v<U>>
using reverse_transform_while_t =
    typeof_t<reverse_transform_while<F, P, T, U, B, E>>;

template <template <typename...> typename F, template <typename> typename P,
          typename T, typename U, auto B = 0, auto E = sizeof_t_v<U>>
struct transform_while_not
    : transform_while<F, negaf<P>::template apply, T, U, B, E> {};
...
template <template <typename...> typename F, template <typename> typename P,
          typename T, typename U, auto B = 0, auto E = sizeof_t_v<U>>
struct reverse_transform_while_not
    : reverse<transform_while_not_t<F, P, T, reverse_t<U>, B, E>> {};

template <template <typename...> typename F, template <typename> typename P,
          typename T, typename U, auto B = 0, auto E = sizeof_t_v<U>>
using reverse_transform_while_not_t =
    typeof_t<reverse_transform_while_not<F, P, T, U, B, E>>;
...
struct logical_or<T, Args...>
    : conditional_of<T, std::true_type, logical_or<Args...>> {};
...
struct logical_and<T, Args...>
    : conditional_of<T, logical_and<Args...>, std::false_type> {};
...
struct is_palindrome {
  ...
  static constexpr auto value =
      and_v<expand_t<impl, T, index_sequence_of_c<N / 2>>>;
};
...
struct one_of {
  ...
  static constexpr auto j = type_if_v < i != E, find_if<F, T, i + 1, E>,
                        c_ < E >>
      ;
...
template <typename T,
          template <typename, typename> typename comparator = less_t>
struct is_sorted_until {
  static constexpr auto N = sizeof_t_v<T>;

  template <size_t i, size_t j>
  struct impl : std::conditional_t<predicate_v<comparator, i, i - 1, T>, c_<i>,
                                   impl<i + 1, j>> {};
...
template <typename T,
          template <typename, typename> typename comparator = less_t>
struct is_sorted
    : std::bool_constant<is_sorted_until_v<T, comparator> == sizeof_t_v<T>> {};
...
struct is_partitioned {
  ...
  struct impl
      : std::conditional_t<visit_v<F, i, T> == B, impl<B, i + 1, j>, c_<i>> {};
  ...
  static constexpr auto value =
      type_if_v<P, c_<l == N ? l : r>, std::bool_constant<l == N || r == N>>;
};
...
template <template <typename...> typename F, typename T>
inline constexpr auto is_partitioned_until_v =
    typev<is_partitioned_until<F, T>>;

template <typename T, typename U, auto B1 = 0, auto E1 = sizeof_t_v<T>,
          auto B2 = 0, auto E2 = sizeof_t_v<U>>
struct is_permutation {
  ...
  struct next {
    ...
    static constexpr size_t n = type_if_v < m == 0, std::false_type, count < V,
                            T, i, E1 >>
        ;
  ...
  struct impl {
    using curr = element_t<i, T>;
    using type = type_if<find_v<curr, T, q, i> != i, impl<i + 1, j>,
                         next<i, curr, impl<i + 1, j>>>;
  };

  template <size_t j>
  struct impl<j, j> : std::true_type {};

  static constexpr auto value = type_if_v < q == E1, std::true_type, impl < q,
                        E1 >>
      ;
};
...
template <typename T, typename U, auto B1 = 0, auto E1 = sizeof_t_v<T>,
          auto B2 = 0, auto E2 = sizeof_t_v<U>>
inline constexpr auto is_permutation_v =
    typev<is_permutation<T, U, B1, E1, B2, E2>>;
...
consteval decltype(auto) is_permutation_of() {
  ...
  return m == n &&
         []<typename F, auto... N>(F&& f, std::index_sequence<N...>) {
           return (f.template operator()<element_t<N, T>>() && ...);
         }([]<typename V>() { return count_v<V, T> == count_v<V, U>; },
           index_sequence_of_c<min_v<m, n>>());
}

template <typename T, typename U, auto B1, auto E1, auto B2, auto E2, auto N,
          bool B, template <typename...> typename F>
struct find_element {
  template <int i, int j, int k>
  struct inner : std::conditional_t<predicate_v<F, k, i, T, U>,
                                    index_upper<B, c_<k>>, inner<i + 1, j, k>> {
  };
...
template <typename T, typename U, auto B1, auto E1, auto B2, auto E2, auto N,
          bool B, template <typename...> typename F>
inline constexpr auto find_element_v =
    typev<find_element<T, U, B1, E1, B2, E2, N, B, F>>;
...
template <typename T, typename U, auto B1 = 0, auto E1 = sizeof_t_v<T>,
          auto B2 = 0, auto E2 = sizeof_t_v<U>,
          template <typename...> typename F = std::is_same>
inline constexpr auto find_first_of_v =
    typev<find_first_of<T, U, B1, E1, B2, E2, F>>;
...
template <typename T, typename U, auto B1 = 0, auto E1 = sizeof_t_v<T>,
          auto B2 = 0, auto E2 = sizeof_t_v<U>,
          template <typename...> typename F = std::is_same>
inline constexpr auto find_first_not_of_v =
    typev<find_first_not_of<T, U, B1, E1, B2, E2, F>>;
...
struct find_first_last_of {
  static constexpr auto value =
      find_first_not_of_v<T, U, B1, E1, B2, E2, F> - 1;
};

template <typename T, typename U, auto B1 = 0, auto E1 = sizeof_t_v<T>,
          auto B2 = 0, auto E2 = sizeof_t_v<U>,
          template <typename...> typename F = std::is_same>
inline constexpr auto find_first_last_of_v =
    typev<find_first_last_of<T, U, B1, E1, B2, E2, F>>;
...
template <typename T, typename U, auto B1 = 0, auto E1 = sizeof_t_v<T>,
          auto B2 = 0, auto E2 = sizeof_t_v<U>,
          template <typename...> typename F = std::is_same>
inline constexpr auto find_last_of_v =
    typev<find_last_of<T, U, B1, E1, B2, E2, F>>;
...
template <typename T, typename U, auto B1 = 0, auto E1 = sizeof_t_v<T>,
          auto B2 = 0, auto E2 = sizeof_t_v<U>,
          template <typename...> typename F = std::is_same>
inline constexpr auto find_last_not_of_v =
    typev<find_last_not_of<T, U, B1, E1, B2, E2, F>>;
...
template <typename T, typename U, auto B1 = 0, auto E1 = sizeof_t_v<T>,
          auto B2 = 0, auto E2 = sizeof_t_v<U>,
          template <typename...> typename F = std::is_same>
inline constexpr auto find_last_first_of_v =
    typev<find_last_first_of<T, U, B1, E1, B2, E2, F>>;
...
struct is_subset {
  ...
  static constexpr auto value =
      type_if_v < N<M, std::false_type, impl<0, N - M + 1>>;
};
...
template <typename T, auto B = 0, auto E = sizeof_t_v<T>,
          template <typename, typename> typename F = std::is_same>
struct unique_copy {
  template <int i, int j, typename prev, typename V>
  struct impl {
    using curr = element_t<i, T>;
    using type = typeof_t<
        impl<i + 1, j, curr, append_if_t<!typev<F<prev, curr>>, V, curr>>>;
  };
  ...
  using type =
      type_if<B == E, base,
              impl<B + 1, E, prev, append_if_t<B != E, clear_t<T>, prev>>>;
};
...
struct cartesian_product {
  ...
  struct impl<i, j, std::index_sequence<N...>> {
    ...
    using type =
        concat_t<conc, typeof_t<impl<i + 1, j, std::index_sequence<N...>>>>;
  };
...
template <size_t N, typename... Args>
constexpr decltype(auto) next_iterator(std::tuple<Args...>& t,
                                       std::tuple<Args...>& b,
                                       std::tuple<Args...>& e) {
  auto& i = ++std::get<N>(t);
...
constexpr decltype(auto) descartes_product(F&& f, Args&&... args) {
  ...
  tuple_for_each(tuple, [&]<typename T>(T&& t) {
    if (!empty && t.begin() == t.end()) empty = true;
  });
  ...
  if (!empty) {
    ...
    for (auto t = b; std::get<0>(t) != std::get<0>(e);
         next_iterator<N - 1>(t, b, e))
...
struct large_number_multiplier {
  ...
  template <int i, typename V, bool = i<sizeof_t_v<V> - 1> struct next {
    static constexpr int m = get_v<i, V>;
    using type =
        typeof_t<next<i + 1, sub_t<i, m % 10, add_t<i + 1, m / 10, V>>>>;
  };
...
template <template <typename...> typename F, typename T,
          template <typename, typename> typename G>
struct adjacent_apply {
  ...
  struct impl {
    using curr = element_t<i, T>;
    using type =
        typeof_t<impl<i + 1, j, curr, typeof_t<G<V, typeof_t<F<U, curr>>>>>>;
  };
  ...
  using type =
      typeof_t <
      impl<N<2 ? N : 1, N,
             element_if_t<!(N < 2), 0, T, std::type_identity<base>>, base>>;
};
...
template <template <typename...> typename F, typename T, bool B,
          template <typename...> typename G, template <typename...> typename H>
struct pairwise_apply {
  ...
  using type =
      typeof_t<G<adjacent_apply_t<impl, T, append_range>, typeof_t<H<T>>>>;
};
...
struct permutation {
  ...
  struct impl {
    static constexpr auto value =
        negav<comparator<element_t<k - 1, T>, element_t<k, T>, ASC>>;
    using type = type_if<k == 1 && value, index_upper<0, T>,
                         impl<k == 1 ? -1 : k - 1, value>>;
  };
  ...
  struct impl<k, false> {
    ...
    using type = index_type<
        1,
        reverse_range_t<n + 1, N, swap_t<find_if_backward_v<comp, T>, n, T>>>;
  };
...
template <size_t N, typename T>
using next_partial_permutation =
    next_permutation<N, reverse_range_t<N, sizeof_t_v<T>, T>>;

template <size_t N, typename T>
using next_partial_permutation_t =
    partial_permutation_t<next_partial_permutation, N, T>;

template <size_t N, typename T>
using next_partial_permutation_list =
    ranged_list_t<next_partial_permutation, T, N>;
...
template <size_t N, typename T>
using prev_partial_permutation_t =
    partial_permutation_t<prev_partial_permutation, N, T>;

template <size_t N, typename T>
using prev_partial_permutation_list =
    ranged_list_t<prev_partial_permutation, T, N>;
...
struct permutation_recursive {
  ...
  struct impl {
    ...
    struct next {
      ...
      using type =
          typeof_t<next<i + 1, j, swap_t<l, i, first_t<call>>, second_t<call>>>;
    };
...
template <typename T, template <typename, auto> typename permute,
          auto N = nest_depth_v<T>>
struct nest_permutation {
  ...
  using type =
      transform_t<impl, typeof_t<permute<to_flat_t<nest_clear_t<T>>, N>>>;
};
...
template <typename T, auto N = nest_depth_v<T>>
struct nest_permutation_recursive
    : nest_permutation<T, permutation_recursive, N> {};
...
struct combination {
  ...
  struct next {
    static constexpr auto cond =
        i - N != B1 && negav<comparator<element_t<i - N, T>, U, ASC ? N : !N>>;
    static constexpr auto value =
        value_if<cond, next<U, M, N, i + M, j>, index_t<i - N>>;
  };
  ...
  struct exch {
    ...
    using type = type_if<value, exch<curr, N, i - 1, j + 1>,
                         std::type_identity<pair_t<U, pair_v<i, j>>>>;
  };
  ...
  struct reverse {
    ...
    using rang =
        reverse_range_t<N, E1,
                        reverse_range_t<N, first_v<call>, first_t<curr>>>;
    using type = index_type<
        !result,
        reverse_range_t<j, E2, reverse_range_t<second_v<call>, E2, rang>>>;
  };
  ...
  struct impl {
    ...
    static constexpr auto result =
        k == B1 && negav<comparator<element_t<B1, T>, lhs, ASC>>;

    using rhs = element_if_t<!result, k, T, std::type_identity<T>>;
    static constexpr auto value =
        value_if<!result, next<rhs, 1, 0, B2, j>, index_t<B2>>;

    using curr = swap_if_t<!result, k, value, T>;
    using call =
        std::conditional_t<!result, pair_v<k + 1, value + 1>, pair_v<B1, B2>>;

    static constexpr auto cond = first_v<call> != E1 && second_v<call> != E2;
    using type =
        type_if<cond, reverse<curr, first_v<call>, E1, second_v<call>, result>,
                index_upper<!result, curr>>;
  };

  using type =
      type_if<B1 == E1 || B2 == E2, index_upper<0, T>, impl<E1, E2 - 1>>;
};
...
template <template <size_t, size_t, typename> typename F, size_t B, size_t E,
...
struct combination_counts {
  ...
  struct calc<i, false> {
    ...
    using cond =
        std::conditional_t<get_v<E - 1, curr>, swap<i + 1, E - 1, curr>,
                           std::type_identity<curr>>;
  ...
  template <size_t N>
  struct impl {
    using cond = ternary_conditional_t<!ASC, N == B, get<N, T>, index_t<1>,
                                       index_t<N == B>>;
    static constexpr auto value =
        value_if<ASC, cond, index_t<N == E - 1 || negav<cond>>>;
  };
...
template <size_t B, size_t E, typename T>
using next_combination_counts_list =
    indices_list_t<next_combination_counts, B, E, T>;
...
template <size_t B, size_t E, typename T>
using prev_combination_counts_list =
    indices_list_t<prev_combination_counts, B, E, T>;
...
struct hypercube_indices {
  ...
  struct impl {
    ...
    using next = std::conditional_t<i != j - 1, impl<curr, i, j - 1>,
                                    index_upper<0, curr>>;
...
template <size_t B, size_t E, typename T>
using next_hypercube_indices_list =
    indices_list_t<next_hypercube_indices, B, E, T>;

template <size_t B, size_t E, typename T>
using prev_hypercube_indices_list =
    indices_list_t<prev_hypercube_indices, B, E, T>;
...
template <size_t N, typename T, template <size_t, typename...> typename F,
          bool B>
struct nest_call {
  using last = nest_last_t<T>;
  ...
  template <typename U>
  using impl = to_nest<
      append_if_t<!is_variadic_type_v<last>, expand_of_t<curr, U>, last>>;

  using type =
      transform_t<impl, F<N, reverse_if_t<B, index_sequence_of_t<curr>>>>;
};
...
template <size_t N, typename T>
using nest_prev_combination_list =
    nest_call_t<N, T, prev_combination_list, true>;

template <size_t N, typename T>
using nest_next_combination_list =
    nest_call_t<N, T, next_combination_list, false>;
...
template <typename T>
using nest_prev_permutation_list =
    nest_call_t<0, T, prev_permutation_call, true>;

template <typename T>
using nest_next_permutation_list =
    nest_call_t<0, T, next_permutation_call, false>;

template <size_t N, typename T>
using nest_prev_partial_permutation_list =
    nest_call_t<N, T, prev_partial_permutation_list, true>;

template <size_t N, typename T>
using nest_next_partial_permutation_list =
    nest_call_t<N, T, next_partial_permutation_list, false>;
...
struct power_set {
  ...
  struct call<std::index_sequence<m...>, std::index_sequence<n...>> {
    template <auto p, auto q>
    using next =
        rename_t<element_if_t<q != 0, N - p - 1, T, std::type_identity<base>>,
                 base>;
  ...
  using type =
      expand_t<impl, next_hypercube_indices_list<0, 2, index_sequence_c<N, 0>>>;
};
...
template <auto N, template <typename, auto...> typename T, typename U,
          auto... values>
...
struct apply_nth {
  ...
  using type = typeof_t<impl<
      arithmetic_sequence_t<N, index_sequence_of_c<M / N + !!(M % N)>>, B>>;
};
...
struct slide_indices {
  ...
  using type = index_type<
      !cond, type_if<cond, std::type_identity<T>, increase<T, 2 * ASC - 1>>>;
};
...
template <typename T, template <typename...> typename F, auto B = 0,
          auto E = sizeof_t_v<T>>
...
struct partition_copy {
  ...
  struct impl {
    ...
    using type = typeof_t<
        impl<i + 1, j, append_if_t<B, U, curr>, append_if_t<!B, V, curr>>>;
  };
...
template <int p, int r, typename T,
          template <typename, typename> typename comparator = less_equal_t>
using randomized_partition_t =
    typeof_t<randomized_partition<p, r, T, comparator>>;

template <int p, int r, int len, int size, typename T,
...
struct partition_adaptive {
  ...
  struct next<j, j, k, V, W> {
    using type = index_type<
        k, concat_t<range_t<0, p, T>, V, W, range_t<r, sizeof_t_v<T>, T>>>;
  };

  template <int m>
  struct split {
    using left_split =
        typeof_t<partition_adaptive<p, m, len / 2, size, T, comparator>>;
    using left_partition = typeof_t<left_split>;

    static constexpr auto half = len - len / 2;
    static constexpr auto index =
        find_if_not_v<comparator, left_partition, m, m + half>;

    using right_partition =
        partition_adaptive<index, r, half, size, left_partition, comparator>;
    using right_split =
        type_if<half, right_partition, index_upper<index, left_partition>>;

    static constexpr auto b = typev<left_split>;
    static constexpr auto e = typev<right_split>;

    using type =
        index_type<b + e - m, rotate_t<b, m, e, typeof_t<right_split>>>;
  };
  ...
  struct impl {
    using base = clear_t<T>;
    using type =
        typeof_t<next<p + 1, r, p, base, append_t<base, element_t<p, T>>>>;
  };
  ...
  using type = type_if<len == 1 || r <= p, index_upper<p, T>,
                       impl<p + len / 2, len <= size>>;
};

template <int p, int r, int len, int size, typename T,
          template <typename...> typename comparator>
using partition_adaptive_t =
    typeof_t<partition_adaptive<p, r, len, size, T, comparator>>;
...
struct reverse_stable_partition
    : reverse<stable_partition_t<p, r, T, comparator>> {};

template <int p, int r, typename T, template <typename...> typename comparator>
using reverse_stable_partition_t =
    typeof_t<reverse_stable_partition<p, r, T, comparator>>;
...
template <int p, int r, typename T,
          template <typename, typename> typename comparator = less_equal_t>
using randomized_stable_partition_t =
    typeof_t<randomized_stable_partition<p, r, T>>;
...
struct nth_pivot {
  ...
  struct impl {
    ...
    using next = std::conditional_t <
                 i<k, impl<p, q, i, curr>, impl<q + 1, r, i - k, curr>>;
...
struct median {
  ...
  struct impl {
    ...
    using type =
        c_<(element_v<(N - 1) / 2, curr> + element_v<N / 2, curr>) / 2>;
  };
...
struct sort_subrange {
  using type =
      type_if<p == q, std::type_identity<T>, partial_sort<p, q, T, comparator>>;
};
...
struct partition_subrange {
  using next =
      type_if<p != 0, nth_element<p, T, comparator>, std::type_identity<T>>;
  using type = type_if<q != sizeof_t_v<T>, nth_element<q, next, comparator>,
...
struct normalize {
  ...
  struct impl<U, false> {
    ...
    using type = type_if<value || N, increase<U, N == 0 ? value : -N>,
                         std::type_identity<U>>;
  };
...
template <
    template <typename, template <typename, typename> typename> typename sort,
...
struct rank_sort<std::integer_sequence<T, values...>> {
  ...
  static constexpr decltype(auto) nth() {
    ...
    ((n = rank<1, values>(),
      k >= n && k < n + rank<0, values>() ? t = values : T()),
     ...);
  ...
  using type =
      decltype([]<size_t... indices>(const std::index_sequence<indices...>&) {
...
struct map_sort<std::tuple<Args...>> {
  ...
  struct impl<std::index_sequence<indices...>, args...> {
    ...
    static constexpr decltype(auto) rankof() {
      ...
      return ((n > value<args>()) + ...) +
             ((n == value<args>() && indices < k) + ...);
    }

    static constexpr decltype(auto) sort() {
      using ranking = mapping<identity<rankof<indices, args>(), args>...>;

      return std::tuple<typeof_t<
          std::decay_t<decltype(get<indices>(std::declval<ranking>()))>>...>{};
    }
  };

  using index = std::index_sequence_for<Args...>;

  using type = decltype([]<size_t... indices, typename... args>(
                            const std::tuple<field<indices, args>...>&) {
...
struct counting_sort {
  ...
  using impl =
      sort<N, typeof_t<assign<0, k, 0, typeof_t<assign<0, N, 1, C>>>>, B>;
...
struct stooge_sort {
  ...
  struct sort<i, j, U, false>
      : swap_if<predicate_v<comparator, j, i, U>, i, j, U> {};
...
template <typename T, typename U, auto m = sizeof_t_v<T> + 1,
          auto n = sizeof_t_v<U> + 1>
...
struct merge_advance {
  ...
  struct impl {
    ...
    using type =
        typeof_t<impl<i + value, j, p + !value, q, m - 1,
  ...
  using type =
      typeof_t<impl<0, sizeof_t_v<T>, 0, sizeof_t_v<U>, N, clear_t<T>>>;
};
...
struct merge_combine {
  ...
  struct impl {
    ...
    using type = typeof_t<
        impl<i + B1 + B2, j, p + B1 + B3, q, append_t<V, typeof_t<next>>>>;
  };
...
struct kmp {
  ...
  struct impl {
    ...
    using type =
        typeof_t<impl<B, curr, V, dest,
                      value_if<!B && n == N, get<n - 1, W>, c_<n>>, q + 1, p>>;
  };
  ...
  using type =
      typeof_t<impl<0, std::index_sequence<>, P, call, 0, 0, sizeof_t_v<T>>>;
};
...
template <typename T, template <typename...> typename U,
          template <typename, auto> typename V, typename... Args, auto... args>
...
struct boyer_moore_horspool {
  ...
  template <int i, int j, typename U>
  struct impl {
    using next = ternary_conditional_t<j == 0, B1, append<U, c_<i>>,
                                       erase<i, i + N, U>, impl<i, j - 1, U>>;
    using type = type_if<is_same_v<j, i + j, P, std::conditional_t<B1, T, U>>,
                         next, std::type_identity<U>>;
  };

  using maps = typeof_t<
      table<N != 1, N - 1, 1, tuple_t<offset<element_t<0, P>, N - 1>>>>;

  template <int i, int j, typename U, bool = i <= j>
  struct search {
    using curr = typeof_t<impl<i, N - 1, U>>;
    using dest =
        element_if_t<N != 1, N + i - 1, std::conditional_t<B1, T, U>, U>;
    ...
    using type = type_if<(lhs || rhs) && B2, std::type_identity<curr>,
                         search<i + typev<cond>, j - rhs * N, curr>>;
  };
  ...
  using type =
      typeof_t<search<0, sizeof_t_v<T> - N,
                      std::conditional_t<!B1, T, std::index_sequence<>>>>;
};
...
constexpr decltype(auto) tuple_exclude(const std::tuple<Args...>& t) {
  return tuple_select<remove_elements_t<std::index_sequence<N...>,
                                        std::index_sequence_for<Args...>>>(t);
}

template <typename P, typename T>
struct nest_erase_subtype
    : to_nest<erase_subtype_t<to_flat_t<P>, to_flat_t<T>>> {};
...
template <typename P, typename T>
inline constexpr auto nest_contains_subtype_v =
    typev<nest_contains_subtype_t<P, T>>;
...
struct eliminate_subtype {
  ...
  struct impl<i, std::index_sequence<j, n...>, V> {
    ...
    using type =
        typeof_t<impl<i + 1, std::index_sequence<n...>, erase_t<k, k + N, V>>>;
  };
...
template <typename P, typename T>
struct nest_eliminate_subtype
    : to_nest<eliminate_subtype_t<to_flat_t<P>, to_flat_t<T>>> {};
...
template <typename P, typename U, typename T>
struct nest_replace_subtype
    : to_nest<replace_subtype_t<to_flat_t<P>, to_flat_t<U>, to_flat_t<T>>> {};

template <typename P, typename U, typename T>
using nest_replace_subtype_t = typeof_t<nest_replace_subtype<P, U, T>>;

template <typename P, typename T,
          template <typename, typename> typename searcher = bmh>
struct number_of {
...
template <typename T, typename U,
          template <typename...> typename F = std::is_same, bool B = false>
...
struct find_group {
  ...
  struct impl {
    ...
    using type =
        type_if<value, std::type_identity<triple<i, n, curr>>, impl<i + 1, j>>;
  };
...
struct find_nth {
  ...
  struct impl<V, false> {
    ...
    static constexpr auto value =
        type_if_v<less_equal_v<c_<N>, type>, element<N - 1, type>,
                  c_<sizeof_t_v<V>>>;
  };
...
struct unordered_match {
  ...
  struct impl {
    ...
    using type =
        typeof_t<impl<tail_t<V>, next, append_t<X, element_t<0, indices>>>>;
  };

  template <typename V, typename W, typename X>
  struct impl<V, W, X, false> : std::type_identity<X> {};

  using type = typeof_t<impl<T, typeof_t<maps<unique_t<T>, std::tuple<>>>,
                             std::index_sequence<>>>;
};
...
struct intersect {
  ...
  template <typename T, typename U, typename... args>
  struct impl<T, U, args...>
      : impl<duplicate_elements_t<concat_t<unique_t<T>, unique_t<U>>>,
             args...> {};
...
struct hanoi {
  ...
  struct impl {
    ...
    using next =
        change_t<to, prepend_t<element_t<to, first>, get<0, last>>, first>;

    using curr = change_t<from, tail_t<last>, next>;
    using type = typeof_t<
        impl<i - 1, curr, use, to, from, append_t<second, c_<from>, c_<to>>>>;
  };
...
struct lis {
  ...
  struct search {
    ...
    static constexpr auto value =
        unary_v<less_t, B + element_v<mid, U>, B + k, T>;

    using type =
        type_if<value, search<mid + 1, j, k, U>, search<i, mid - 1, k, U>>;
  };
  ...
  struct update<i, j, U, false> {
    using type = change_if_t<unary_v<less_t, B + i, B + element_v<j, U>, T>, j,
                             c_<i>, U>;
  };
  ...
  struct impl {
    ...
    using type =
        typeof_t <
        impl<i + 1, j, max_v<l, k>, change_c<i, element_v<l - 1, V>, U>,
             typeof_t<update<i, l, V, k<l>>>>;
  };

  template <int j, int k, typename U, typename V>
  struct impl<j, j, k, U, V>
      : std::type_identity<triple<k, element_v<k, V>, U>> {};

  using call = typeof_t<impl<B, E, 0, fill_c<E, 0>, fill_c<E + 1, 0>>>;

  template <int i, int j, typename U>
  struct reconstruct : reconstruct<i - 1, element_v<j, typeof_t<call>>,
                                   change_t<i, c_<B + j>, U>> {};
  ...
  using type =
      typeof_t<reconstruct<l - 1, second_v<call>, index_sequence_c<l, 0>>>;
};
...
struct lcs {
  ...
  struct search
      : std::type_identity<pair_t<c_0, c_<get_matrix_element_v<i, j, V> + 1>>> {
  };
  ...
  struct search<i, j, V, false> {
    ...
    static constexpr auto value =
        get_matrix_element_v<i + 1, j, V> <= typev<curr>;

    using type = pair_t<c_<value ? 1 : 2>,
                        type_if<value, curr, get_matrix_element<i + 1, j, V>>>;
  };
  ...
  struct impl {
    ...
    using call =
        prepend_if_t<value == 0, W,
                     element_if_t<value == 0, i, T, std::type_identity<T>>>;
  ...
  using maps = typeof_t<next<0, row, 0, col, indices_table<row, col>,
                             indices_table<row + 1, col + 1>>>;
...
struct manacker {
  ...
  struct odd {
    ...
    struct impl {
      ...
      using type = type_if < value,
            impl<m, j + m - i<N && j - m >= 0>, c_<n - 1>>;
    };
    ...
    using type = pair_t<index_type<j, change_c<j, mid, U>>,
                        triple<lhs, rhs, pair_v<j - mid, j + mid + !i>>>;
  };
  ...
  struct even {
    ...
    struct impl {
      ...
      using type = type_if<value, std::type_identity<call>,
                           impl<p + 1, q, first_v<curr>, second_v<curr>, maps>>;
    };
  ...
  struct impl {
    ...
    using dest = type_if < B,
          std::conditional<lhs<rhs, part, V>, append_if<rhs != 0, V, rang>>;
    ...
    using next = impl<i + value, typev<curr> + 1, first_v<trip>, second_v<trip>,
                      maps, dest>;
    using type = type_if<rhs == 0, std::type_identity<dest>,
                         std::conditional_t<value, prev, next>>;
  };

  using type =
      typeof_t<impl<0, 0, 0, -1, table,
                    std::conditional_t<B, pair_v<0, 0>, std::tuple<>>>>;
};
...
struct lcp {
  ...
  template <int i, int j, typename U, typename V, typename W,
            bool = i<j> struct next {
    using curr = element_t<i, U>;
    static constexpr auto value = rcall_v<std::is_same, i, curr, V>;

    using type =
        type_if<value, next<i + 1, j, U, V, append_if_t<value, W, curr>>,
                std::type_identity<W>>;
  };
  ...
  struct impl {
    ...
    using type = typeof_t<
        next<0, min_v<sizeof_t_v<lhs>, sizeof_t_v<rhs>>, lhs, rhs, base>>;
  };
...
struct lbs {
  ...
  struct next {
    ...
    using curr = element_t<j, U>;
    static constexpr auto value =
        typev<lhs> < typev<rhs> && less_v<element_t<i, U>, curr>;
  ...
  struct next<i, i, k, l, U> {
    ...
    using type =
        typeof_t<next<i + l, l == 1 ? 0 : N - 1, k, l, change_t<i, curr, U>>>;
  };
  ...
  struct impl {
    static constexpr auto sum =
        element_size_v<i, lis> + element_size_v<i, lds> - 1;
  ...
  struct impl<j, j, k, m> {
    ...
    using type = concat_t<range_t<0, sizeof_t_v<lhs> - 1, lhs>,
                          range_t<0, sizeof_t_v<rhs>, rhs>>;
  };
...
struct main_lorentz {
  ...
  struct zfunc {
    ...
    template <int i, int j, int k, int l, typename V>
    struct impl {
      template <typename W, int m = element_v<i, W>,
                bool = i + m<N> struct call {
        static constexpr auto value = is_same_v<m, i + m, U>;
        using type = type_if<value, call<change_if_c<value, i, m + 1, W>>,
                             std::type_identity<W>>;
      };
      ...
      using curr =
          typeof_t<call<change_if_c<i <= l, i, min_v<l - i + 1, q>, V>>>;
  ...
  template <bool B, typename U, typename V, int M,
            typename W = std::conditional_t<B, U, V>>
  using getz = type_if < 0 <= M&& M<sizeof_t_v<W>, element<M, W>, c_0>;

  template <typename U, typename V, int shift = 0, auto N = sizeof_t_v<U>,
            bool = N != 1>
  struct impl {
    ...
    struct call {
      ...
      static constexpr auto p =
          typev<getz<value, lcpa, lcpc, half - i + !value * (half + rest)>>;
      static constexpr auto q =
          typev<getz<value, lcpb, lcpd, i + 1 + value ? rest : -half>>;

      using curr = next < shift, i, l, p, q, W, i<half>;
      using type = typeof_t<
          call<i + 1, j, type_if<l <= p + q, curr, std::type_identity<W>>>>;
    };
    ...
    using type = typeof_t<
        call<0, N,
             typeof_t<impl<rhs, typeof_t<impl<lhs, V, shift>>, shift + half>>>>;
  };
...
template <template <typename...> typename F, typename... Args>
inline constexpr auto conjunction_invoke_v =
    typev<conjunction_invoke_t<F, Args...>>;
...
template <template <typename...> typename F, typename... Args>
inline constexpr auto disjunction_invoke_v =
    typev<disjunction_invoke_t<F, Args...>>;
